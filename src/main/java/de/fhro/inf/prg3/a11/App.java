...
public class App {
    ...
    private static void printCanteens() throws ExecutionException, InterruptedException {
        ...
        openMensaAPI.getCanteens().thenApply(response->{
            ...
            List<Canteen> innerCanteens = new LinkedList<>();
            for (int i = 2; i <= pageInfo.getTotalCountOfPages(); i++){
                try {
                    List<Canteen> canteens = openMensaAPI.getCanteens(i).get();
                    innerCanteens.addAll(canteens);
                } catch (InterruptedException | ExecutionException e) {
                    e.printStackTrace();
                }
            }
            System.out.println(innerCanteens);
            return 0;
        });
